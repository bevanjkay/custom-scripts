...
if (!username || !password) {
  console.log("Please provide FLURO_USERNAME and FLURO_PASSWORD in the environment");
  Deno.exit(1);
}
...
plans.forEach((plan: Plan) => {
  plan.schedules.forEach(schedule => {
    ...
    if (schedule.title.match(regex)) {
      ...
      console.log(schedule.title, schedule.key, removeHTML(notes["Person Responsible"]));
    }
  })
})